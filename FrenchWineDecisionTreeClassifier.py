...
plt.figure(figsize=(20, 10)) #利用seaborn库绘制三种不同参数图
...
# 使用训练的模型进行预测，为了方便，
# 案例直接把测试集里面的数据拿出来三条
print(X_test[1:6])#利用3个数据进行测试，即取3个数据作为模型的输入端
model.predict(X_test[1:6])
print(model.predict(X_test[1:6]))#输出测试的结果，即输出模型预测的结果
